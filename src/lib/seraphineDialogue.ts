...
/** Union of all component keys (used as the hook's first argument) */
export type ComponentKey = keyof typeof DIALOGUE_MAP;

/** Flat union of every possible state string across all components */
export type DialogueState =
  | keyof (typeof DIALOGUE_MAP)['welcomePremium']
  | keyof (typeof DIALOGUE_MAP)['premiumGate']
  | keyof (typeof DIALOGUE_MAP)['billingHistory']
  | keyof (typeof DIALOGUE_MAP)['subscribeTab']
  | keyof (typeof DIALOGUE_MAP)['manageSubscription'];

// ─── Art Slots ───────────────────────────────────────────────
// Maps each component to the CinematicArtSlot names it uses.
// Extend these as real assets are added.

export const ART_SLOTS: Record<ComponentKey, string[]> = {
  welcomePremium:    ['premium'],
  premiumGate:       ['gate'],
  billingHistory:    ['billing'],
  subscribeTab:      ['altar', 'premium'],
  manageSubscription:['altar'],
};
...
export function getDialogue(
  ...
  const stateMap = DIALOGUE_MAP[component] as Record<string, DialogueEntry>;
  const raw: DialogueEntry = stateMap[state] ?? {
    text: '...', useName: false, isSubtle: true,
  };
